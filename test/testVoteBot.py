...
async def run():
    ...
    bot = VoteBot(peoplePool=pool, botdir_path=str(Path(__file__).parent / "botdir"), autoJoin=True, autoLeave=True)

    # run timevents and interactiveRun concurrently
    await asyncio.gather(pool.runTimeEvents(), bot.run(), interactiveRun(pool, votebot=bot))
